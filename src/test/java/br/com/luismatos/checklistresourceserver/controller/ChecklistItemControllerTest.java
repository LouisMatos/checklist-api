...
public class ChecklistItemControllerTest {
	...
	private ChecklistItemDTO getChecklistItemDTO(String description, Boolean isCompleted, LocalDate deadline,
			String categoryName) {
		return ChecklistItemDTO.builder().guid(UUID.randomUUID().toString()).description(description)
				.isCompleted(isCompleted).deadline(deadline)
				.category(CategoryDTO.builder().guid(UUID.randomUUID().toString()).name(categoryName).build())
				.postedDate(LocalDate.now()).build();
	}
	...
	public void shouldCallGetAllChecklistItemsAndReturn200() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.get("/v1/api/checklist-items")).andDo(print())
				.andExpect(status().isOk()).andExpect(jsonPath("$[*]", hasSize(2)))
				.andExpect(jsonPath("$[0].guid").isNotEmpty()).andExpect(jsonPath("$[0].isCompleted").value(false))
				.andExpect(jsonPath("$[0].description").value("Item 1"))
				.andExpect(jsonPath("$[0].deadline").value("2021-10-01"))
				.andExpect(jsonPath("$[0].postedDate").isNotEmpty()).andExpect(jsonPath("$[0].category").isNotEmpty())
				.andExpect(jsonPath("$[0].category.guid").isNotEmpty())
				.andExpect(jsonPath("$[0].category.name").value("Cat 1")).andExpect(jsonPath("$[1].guid").isNotEmpty())
				...
				.andExpect(jsonPath("$[1].postedDate").isNotEmpty()).andExpect(jsonPath("$[1].category").isNotEmpty())
	...
	public void shouldCallEndpointAndAddNewChecklistItemAndReturn201() throws Exception {
		...
		// having
		when(this.checklistItemService.addChecklistItem(anyString(), anyBoolean(), any(LocalDate.class),
				anyString())).thenReturn(
						getChecklistItemEntity(1L, "Item 1", false, LocalDate.of(2021, 10, 01), 1L, "Test Cat"));

		ChecklistItemDTO requestPayload = getChecklistItemDTO("Test", true, LocalDate.now(), "Test Cat");

		// when - then
		this.mockMvc.perform(MockMvcRequestBuilders.post("/v1/api/checklist-items")
				.content(objectMapper.writeValueAsString(requestPayload)).contentType(MediaType.APPLICATION_JSON))
				.andDo(print()).andExpect(status().isCreated()).andExpect(jsonPath("$.guid").isNotEmpty());

		verify(checklistItemService, times(1)).addChecklistItem(anyString(), anyBoolean(), any(LocalDate.class),
				anyString());
	}
	...
	public void shouldCallEndpointAndAddUpdateChecklistItemAndReturn204() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.put("/v1/api/checklist-items")
				.content(
						objectMapper.writeValueAsString(getChecklistItemDTO("Test", true, LocalDate.now(), "Test Cat")))
